...
template<ITEM_TYPE it, uint32_t value>
...
// Main Items
template <uint32_t data> requires ((data&~0x17F)==0) using Input = PRIV::ITEM_UNSIGN_VALUE<PRIV::ITEM_TYPE::Input, data>;
template <uint32_t data> requires ((data&~0x1FF)==0) using Output = PRIV::ITEM_UNSIGN_VALUE<PRIV::ITEM_TYPE::Output, data>;
template <uint32_t data> requires ((data&~0x1FF)==0) using Feature = PRIV::ITEM_UNSIGN_VALUE<PRIV::ITEM_TYPE::Feature, data>;
//   Collections
...
template<int32_t min, int32_t max> requires (min<max)
using LogicalMinMax = PRIV::BUF_COLLECTOR<LogicalMin<min>,LogicalMax<max>>;
template<int32_t min, int32_t max> requires (min<max) using LogicalMinMax = PRIV::BUF_COLLECTOR<LogicalMin<min>,LogicalMax<max>>;
...
template<int32_t min, int32_t max> requires (min<max)
using PhysicalMinMax = PRIV::BUF_COLLECTOR<PhysicalMin<min>,PhysicalMax<max>>;
...
template<uint8_t min, uint8_t max> requires (min<max) using UsageMinMax = PRIV::BUF_COLLECTOR<UsageMin<min>,UsageMax<max>>;
...
template<uint32_t min, uint32_t max> requires (min<max) using DesignatorMinMax = PRIV::BUF_COLLECTOR<DesignatorMin<min>,DesignatorMax<max>>;
...
template<uint32_t min, uint32_t max> requires (min<max) using StringMinMax = PRIV::BUF_COLLECTOR<StringMin<min>,StringMax<max>>;
